...
public class ConfluenceReleaseNotesPublisher extends AbstractJIRAConfluenceAction {
    ...
    public ConfluenceReleaseNotesPublisher(String jiraCredentialsID, String confluenceCredentialsID, String spaceKey,
            String jqlFilter, String pageTitle, String pageLabels, String parentPageTitle) {
    ...
    public boolean perform(AbstractBuild< ?, ? > build, Launcher launcher, BuildListener listener)
        ...
        JIRAAccessTool jira = new JIRAAccessTool(jiraCred.getUrl(), jiraCred.getUrlUsername(), jiraCred.getUrlPassword(), null,
                jiraCred.getProjectKey(), getJiraCredentialsID());
        ConfluenceAccessTool confluence = new ConfluenceAccessTool(confCred.getUrl(), confCred.getUrlUsername(),
                confCred.getUrlPassword(), null);
        ...
        String pageLabels = varReplacer.replace(this.pageLabels);
        String[] labels = null;
        if (pageLabels != null && !pageLabels.isEmpty()) {
            labels = pageLabels.split(" ");
        }

        List< Issue > tickets = jira.getTicketsByJQL(jqlFilter);

        logger.println(
                "Publishing " + tickets.size() + " tickets on page '" + pageTitle + "' in space '" + spaceKey + "' on confluence.");

        String pageHTML = new String(jira.buildReleaseNotesHTML(tickets).getBytes("UTF-8"), "ISO8859_1");
        ...
        confluence.createPage(pageTitle, pageHTML, spaceKey, parentPageID, labels);
